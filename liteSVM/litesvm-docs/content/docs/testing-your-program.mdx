...
fn test_my_program() {
    ...
    // Deploy your program
    let program_id = Pubkey::new_unique(); // Or use your program's actual ID
...
<Callout type="tip">
  When deploying programs, `include_bytes!` embeds the binary directly into the
  test executable at compile time, so there is no I/O during test
  execution.{" "}
</Callout>
...
<Callout type="tip">
  LiteSVM includes SPL Token programs by default, so you don't need to deploy
  them manually.
</Callout>
